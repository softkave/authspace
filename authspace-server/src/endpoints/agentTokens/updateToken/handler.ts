...
const updateAgentToken: UpdateAgentTokenEndpoint = async (
  context,
  instData
) => {
  const data = validate(instData.data, updateAgentTokenJoiSchema);
  const agent = await context.session.getAgent(context, instData);
  const {workspace} = await tryGetWorkspaceFromEndpointInput(
    context,
    agent,
    data
  );
  ...
  const updatedToken = await executeWithTxn(context, async opts => {
    ...
    [token] = await Promise.all([
      context.semantic.agentToken.getAndUpdateOneById(
        token.resourceId,
        tokenUpdate,
        opts
      ),
