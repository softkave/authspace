...
describe('resolveEntityPermissions', () => {
  test('correct results returned', async () => {
    ...
    await insertPermissionItemsForTest(
      context,
      userToken,
      workspace.resourceId,
      [pItem01, pItem02, pItem03]
    );
...
function indexResolvedPermissions(item: ResolvedPermissionItem) {
  return makeKey([
    item.entityId,
...
function indexResolvedPermissionItemTarget(item: ResolvedPermissionItemTarget) {
