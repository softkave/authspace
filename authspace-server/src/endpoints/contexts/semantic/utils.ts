  ...
  implements SemanticDataAccessBaseProviderType<T>
{
  ...
  async insertWithQuery(
    ...
    return await this.memstore.createWithQuery(
      queryFn,
      itemsFn,
      opts.transaction
    );
  ...
  async updateOneById(
    ...
    const query: LiteralDataQuery<Resource> = {resourceId: id};
    return await this.memstore.updateItem(
      query as LiteralDataQuery<T>,
      update,
      opts.transaction
    );
  ...
  async deleteOneById(
    ...
    const query: LiteralDataQuery<Resource> = {resourceId: id};
    await this.memstore.deleteItem(
      query as LiteralDataQuery<T>,
      opts.transaction
    );
  }
  ...
  async getManyByIdList(
    idList: string[],
    options?:
      | (IDataProvideQueryListParams<T> & SemanticDataAccessProviderRunOptions)
      | undefined
  ): Promise<T[]> {
    const query: LiteralDataQuery<Resource> = {resourceId: {$in: idList}};
    ...
    const query: LiteralDataQuery<SemanticDataAccessWorkspaceResourceProviderBaseType> =
      {
        ...
        resourceId: {
          ...
          $nin: q.excludeResourceIdList?.length
            ? q.excludeResourceIdList
            : undefined,
        },
      };
    return await this.memstore.countItems(
...
export async function executeWithTxn<T>(
