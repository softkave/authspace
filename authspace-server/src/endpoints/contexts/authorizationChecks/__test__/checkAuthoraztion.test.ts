...
import {executeWithTxn} from '../../semantic/utils';
...
describe('checkAuthorization', () => {
  ...
  test('auth fails when agent does not have permission', async () => {
    ...
    const agent02 = await context.session.getAgent(
      context,
      RequestData.fromExpressRequest(
        mockExpressRequestWithAgentToken(userToken02)
      )
    );
  ...
  test('auth passes if action is read and user is not email verified', async () => {
    ...
    await executeWithTxn(context, opts =>
      assignWorkspaceToUser(
        context!,
    ...
    await grantEveryPermission(
      workspace,
      userToken,
      userToken02.separateEntityId
    );
    await checkAuthorization({
      context,
      workspace,
  ...
  test('auth with appliesTo', async () => {
    ...
    const [outerFolder01, outerFolder02, outerFolder03, outerFolder04] =
      outerFolders;
    ...
    await expectErrorThrown(async () => {
      ...
      await checkAuthorization({
        ...
        action: AppActionType.Read,
        containerId: getFilePermissionContainers(
          workspace.resourceId,
          outerFolder01
        ),
      });
