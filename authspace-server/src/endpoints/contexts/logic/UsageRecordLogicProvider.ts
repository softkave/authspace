...
export class UsageRecordLogicProvider {
  insert = async (
    ...
    const workspace = await ctx.semantic.workspace.getOneById(
      record.workspaceId
    );
  ...
  private async getUsagel2(
    ...
    return await executeWithTxn(context, async opts => {
  ...
  private checkExceedsRemainingUsage = async (
    ...
    return await executeWithTxn(context, async opts => {
      let [usageFulfilledL2, usageTotalFulfilled, usageDroppedL2] =
        await Promise.all([
          this.getUsagel2(
            context,
            agent,
      ...
      const totalMonthUsageThreshold =
        workspace.usageThresholds[UsageRecordCategory.Total];
      ...
      const projectedUsageCost = getCostForUsage(
        record.category,
        projectedUsage
      );
      if (
