...
type FimidaraExternalError = Pick<
  OperationError,
...
export function getWorkspaceResourceListQuery(
  workspace: Workspace,
  permissionsSummaryReport: Awaited<
    ReturnType<typeof summarizeAgentPermissionItems>
  >
) {
...
export async function executeCascadeDelete<Args>(
  ...
  const helperFns: DeleteResourceCascadeFnHelperFns = {
    async withTxn(fn: AnyFn<[SemanticDataAccessProviderMutationRunOptions]>) {
      await executeWithTxn(context, opts => fn(opts));
    },
  };

  await Promise.all(
    Object.values(cascadeDef).map(fn => fn(context, args, helperFns))
  );
}
