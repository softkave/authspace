...
import {AssignEndpoint} from './types';
import {assignJoiSchema} from './validation';

const assign: AssignEndpoint = async (context, instData) => {
  const data = validate(instData.data, assignJoiSchema);
