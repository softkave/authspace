...
import {
  executeServerInstanceJobs,
  waitForServerInstanceJobs,
} from '../../jobs/runner';
...
export function mutationTest(
  ...
  executeWithTxn(context, async options => {
  ...
  if (context) {
    await Promise.all(
      toNonNullableArray(context).map(async context => {
        if (context) {
          await executeServerInstanceJobs(
            context,
            context.appVariables.serverInstanceId
          );
          await waitForServerInstanceJobs(
            context,
            context.appVariables.serverInstanceId
          );
