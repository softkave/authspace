...
export const appMessages = {
  ...
  user: {
    ...
    configNotFound(id?: string) {
      return id
        ? `Users config with ID ${id} not found.`
        : 'Users config not found.';
    },
    changePassword() {
  ...
  dataSchema: {
    notFound(id?: string) {
      return id
        ? `Data schema with ID ${id} not found.`
        : 'Data schema not found.';
    },
    exists(id?: string) {
      return id ? `Data schema with ID ${id} exists.` : 'Data schema exists.';
    },
    existsWithName(name?: string) {
      return name
        ? `Data schema with name ${name} exists.`
        : 'Data schema exists.';
    },
  },
  common: {
